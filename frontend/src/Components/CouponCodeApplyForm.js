...
export default function CouponCodeApplyForm({
    ...
    const submitCouponCode = async (e) => {
        ...
        const verifyDiscountCoupon = async (code) => {
            try {
                const response = await axios.get(`${BASE_URL}/coupons/verify-coupon/${code}`, {
                    params: {
    ...
    }, [subtotalAmount, couponcode, discountAmount, totalAmount, setCheckOutFormData]);

    return (
        <Form onSubmit={submitCouponCode} className='d-flex align-items-start gap-2'>
            <TextField
                required
                size="small"
                label="Coupon Code"
                ...
                className={`text-uppercase w-100 border-dark`}
                value={couponcode}
