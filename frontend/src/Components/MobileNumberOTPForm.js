...
export default function MobileNumberOTPForm({ userId, currentStep, setCurrentStep, checkOutFormData, setCheckOutFormData }) {
    ...
    const handleMobileNumberSubmit = () => {
        if (/^\d{10}$/.test(mobileNumber)) {
            ...
            const sendOTP = async () => {
                try{
                    const response = await axios.post(`${BASE_URL}/user/login-otp`, { mobileNumber });
            ...
            setMobileNumberError('Incorrect entry. Please enter a valid 10-digit mobile number.');
        }
    };
            ...
            <Form
                ...
                <Grid item xs={12}>
                    <Button
                        ...
                        disabled={loading}
                        >
                        { loading ? "Processing req..." : ( showOTPField ? 'Verify OTP' : 'Submit Number' ) }
                    </Button>
                </Grid>
            </Form>
        </Grid>
